...
class Home extends Component {
  ...
  handleSaveButton = article => {
    ...
    console.log("I am trying to save", newArticle);

    API.saveArticle(newArticle)
      .then(res => {
        console.log(res);
        //filter out the saved article from the array of results in state
        let remainingArticles = this.state.articles.filter(
  ...
  handleFormSubmit = event => {
    ...
    if (this.state.query && this.state.startYear && this.state.endYear) {
      console.log("im about to retrieve them");
      API.searchNYTArticles(
        this.state.query,
        ...
        .then(res => {
          ...
          console.log("i finished retrieving them");
        })
        .catch(err => console.log(err));
    }
  };
